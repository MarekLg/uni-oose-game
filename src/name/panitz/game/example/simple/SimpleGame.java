...
public class SimpleGame<I, S> extends AbstractGame<I, S> {
	...
	TextObject<I> infoText = new TextObject<>(new Vertex(30, 35), "Kleines Beispielspiel", "Helvetica", 28);

	SoundObject<S> outch = new SoundObject<>("crash.wav");

	public SimpleGame() {
		super(new ImageObject<>("hexe.png", new Vertex(200, 200), new Vertex(1, 1)), 800, 600);
		...
		getGOs().addAll(hintergrund);
		getGOs().addAll(wolken);
		gos.addAll(gegner);
